  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          title: Text('entertainment',style: TextStyle(color: Colors.white,fontWeight: FontWeight.w700,fontSize: 30),)
                      ...
                      itemBuilder: (context,index){return
                        GestureDetector(
                          child: Container(
                            ...
                            child:Text('${Provider.of<Newsprovider>(context).entertainmentdata[index]['source']['name']??''}'
            ...
            Expanded(flex: 15,
              child: ListView.separated(itemBuilder: (context,index){
                return Column(
                  ...
                  onTap: (){
                    Navigator.push(context, MaterialPageRoute(builder: (ctx){
                      return Newsdetails(content: Provider.of<Newsprovider>(context).entertainmentdata[index]['description']??'',
                          image: Provider.of<Newsprovider>(context).entertainmentdata[index]['urlToImage']??'',
                          source: Provider.of<Newsprovider>(context).entertainmentdata[index]['source']['name']??'',
                    ...
                    Text(Provider.of<Newsprovider>(context).entertainmentdata[index]['source']['id']??'',style: TextStyle(color: Colors.grey),),
                    SizedBox(height: MediaQuery.of(context).size.height*.02,),
                     Text(Provider.of<Newsprovider>(context).entertainmentdata[index]['title']??'',style: TextStyle(color: Colors.black,fontSize: 25,),
                      maxLines:4,
                      overflow: TextOverflow.ellipsis,),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                      children: [
                        Text(Provider.of<Newsprovider>(context).entertainmentdata[index]['publishedAt']??'',style: TextStyle(color: Colors.grey),)
                      ],
                    )
                  ],
                );
                  ...
                  itemCount: Provider.of<Newsprovider>(context).entertainmentdata.length),
            ),
          ],
        ),
      ),
