  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Business',style: TextStyle(color: Colors.white,fontWeight: FontWeight.w700,fontSize: 30),)

      ),

      body:   Provider.of<Newsprovider>(context).Businessdata.isEmpty?
      Center(
                      ...
                      itemBuilder: (context,index){return
                        GestureDetector(
                          child: Container(
                            ...
                            child:Text('${Provider.of<Newsprovider>(context).Businessdata[index]['source']['name']??''}'
                ...
                return Column(
                  ...
                  children: [
                    GestureDetector(
                      onTap: (){
                        Navigator.push(context, MaterialPageRoute(builder: (ctx){
                          return Newsdetails(content: Provider.of<Newsprovider>(context).Businessdata[index]['description']??'',
                              image: Provider.of<Newsprovider>(context).Businessdata[index]['urlToImage']??'',
                              source: Provider.of<Newsprovider>(context).Businessdata[index]['source']['name']??'',
                    ...
                    Text(Provider.of<Newsprovider>(context).Businessdata[index]['source']['id']??'',style: TextStyle(color: Colors.grey),),
                    SizedBox(height: MediaQuery.of(context).size.height*.02,),
                    Text(Provider.of<Newsprovider>(context).Businessdata[index]['title']??'',style: TextStyle(color: Colors.black,fontSize: 25,),
                      maxLines:4,
                      overflow: TextOverflow.ellipsis,),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                      children: [
                        Text(Provider.of<Newsprovider>(context).Businessdata[index]['publishedAt']??'',style: TextStyle(color: Colors.grey),)
                      ],
                    )
                  ],
                );
